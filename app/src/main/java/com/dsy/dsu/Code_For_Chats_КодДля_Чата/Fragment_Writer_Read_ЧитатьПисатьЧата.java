...
// TODO: 01.07.2021 метод когда нет данных в курсоре и отображаем сообщени что сообщений нет в фрагменте
    ...
    class CONTROLLER implements AdapterView.OnItemClickListener {
        /////
            ...
            try {
                ...
                if (ПолученыйУжеСуществующийUUIDИзПерепискиДляЧата == 0) {
                    ...
                    ПолученыйУжеСуществующийUUIDИзПерепискиДляЧата
                            ...
                            .МетодПовторноПроверетНеПовилосьЛиМеждеУчаникамиперепискиПубличныйUUID(getContext(),ПолученыйIDДляЧата,ПубличныйIDДляФрагмента);
...
*/
        ...
        private class SubClass_RetryGEtRowInChatsКлассПроверемЕщеРАзПоявилосЛИПуббличныйUUIDМеждуУчасникамиЧата {


            Long МетодПовторноПроверетНеПовилосьЛиМеждеУчаникамиперепискиПубличныйUUID(Context context
                    , Long ПолученыйIDДляЧата, Integer ПубличныйIDДляФрагмента) {
                ...
                // TODO: 05.07.2021 после успешной вставки новой записи обновляем UI
                    ...
                    Class_GRUD_SQL_Operations class_grud_sql_operations_ДляПовторногоПоискаПуцбличногоUUIDМеждуУчасникамиперпискиЧатаТекущего  = new Class_GRUD_SQL_Operations(getActivity());
                    ///
...
*/
                    ...
                    КурсорДанныеИщемНЕПоявильсяЛиПубличныйUUIDМеджуУчасникамиПереписки = (SQLiteCursor) class_grud_sql_operations_ДляПовторногоПоискаПуцбличногоUUIDМеждуУчасникамиперпискиЧатаТекущего.
                            new GetаFreeData(getActivity()).getfreedata(class_grud_sql_operations_ДляПовторногоПоискаПуцбличногоUUIDМеждуУчасникамиперпискиЧатаТекущего.concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций,
                            Class_Engine_SQLГдеНаходитьсяМенеджерПотоков.МенеджерПотоков, Create_Database_СсылкаНАБазовыйКласс.getССылкаНаСозданнуюБазу());


                    if (КурсорДанныеИщемНЕПоявильсяЛиПубличныйUUIDМеджуУчасникамиПереписки!=null) {
