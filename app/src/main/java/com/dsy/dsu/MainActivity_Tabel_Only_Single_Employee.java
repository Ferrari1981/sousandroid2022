...
/*        // TODO: 09.04.2021 запуск синхронизации фоновой по расписанию
        ...
        boolean       РезультатЕслиСвязьСерверомПередНачаломВизуальнойСинхронизции=false;
            ...
            РезультатЕслиСвязьСерверомПередНачаломВизуальнойСинхронизции=
                    new Class_MODEL_synchronized(getApplicationContext()).УниверсальнайМетодПроверкиПодключениекWIFIДляСлужбыТолькоДляЧата(getApplicationContext());
...
// TODO: 01.07.2021  после локальной обнолвения поробуем вотрунть синхронизацию локальную  в фоне и порстмортрим что будет
            Integer РезультатФоновойСинхронизации =
                    new Class_MODEL_synchronized(getApplicationContext()).МетодЗапускаФоновойСинхронизацииДополнительноНаSingleТабеле(getApplicationContext());
...
///TODO СЮДА ЗАХОДИМ ПРОСТО БЕЗ СОЗДАНИЕ НОВОГО ЗАГРУЖАЕТ УЖЕ СОЗДАННОЕ ТАБЕЛЯ В БАЗЕ "status_send", "Удаленная"
            ...
            Курсор_ПолучаемКоличествоСотрудниковВАтбеле =
                    new Class_MODEL_synchronized(getApplicationContext())
                            .МетодЗагружетУжеготовыеТабеляДляСкролаПОТабелюТолькоКоличествоСТорочек(КонтекстОдногоСотрудикаВТабеле,
            ...
            if ( ПроизошелЛиСфайпПоДаннымСингТабеля==false) {
                ...
                ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество =
                        new Class_MODEL_synchronized(getApplicationContext()).МетодЗагружетУжеготовыеТабеля(КонтекстОдногоСотрудикаВТабеле,
...
/////////////////////////
                ...
                if (UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников == 0) {
                    ...
                    ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество =
                            new Class_MODEL_synchronized(getApplicationContext()).
                                    МетодЗагружетУжеготовыеТабеляПриСмещенииДанныхСкроллПоДАнным(КонтекстОдногоСотрудикаВТабеле,
                                            ЦифровоеИмяНовгоТабеля, МЕсяцДляКурсораТабелейДЛяПермещения, ГодДляКурсораТабелей);
                    ...
                    ////////////////////
                    ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество = new Class_MODEL_synchronized(getApplicationContext()).МетодЗагружетУжеготовыеТабеля(КонтекстОдногоСотрудикаВТабеле,
                            UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников, МЕсяцДляКурсораТабелейДЛяПермещения, ГодДляКурсораТабелей);
