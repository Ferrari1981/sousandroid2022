...
public class MainActivity_Tabel_Only_Single_Employee extends AppCompatActivity  {
    ...
    Map< Integer,String> ХЭШНазваниеДнейНедели = Collections.synchronizedMap(new LinkedHashMap<>());
    ...
    TextView ОбщееКоличествоЧасовСотрудникаВТабелеСотудников;
...
//private   SQLiteCursor ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп = null;
    ...
    Context КонтекстОдногоСотрудикаВТабелеВнешний;


    Context КонтекстОдногоСотрудикаВТабелеДляСинхронизации;
...
// TODO: 06.05.2021
    ...
    boolean  ПроизошелЛиСфайпПоДаннымСингТабеля;
    ...
    Observer observerОдноразоваяВнутриТабеля;
...
///////TODO
            ...
            Log.d(this.getClass().getName(), "  onCreate(Bundle savedInstanceState)   MainActivity_Tabel_Only_Single_Employee  ");
...
/*        // TODO: 09.04.2021 запуск синхронизации фоновой по расписанию
            ...
            final boolean[] ПроверкаБылиИзмененияВБазе = {false};
            //
            LinkedBlockingQueue<String> linkedBlockingQueueДвеТаблитцызапускаПриМинхрониазцииСТАбеля=new LinkedBlockingQueue();
            ////
            linkedBlockingQueueДвеТаблитцызапускаПриМинхрониазцииСТАбеля.offer("tabel");
            ///
            linkedBlockingQueueДвеТаблитцызапускаПриМинхрониазцииСТАбеля.offer("data_tabels");

            //
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ////
                linkedBlockingQueueДвеТаблитцызапускаПриМинхрониазцииСТАбеля.spliterator().forEachRemaining((ТекущаяОбрабоатываеммаяТаблицыДлВнутренгоСинхрониазуииВТАбеле)->{
                    ///todo

                    ПроверкаБылиИзмененияВБазе[0] =
                            new Class_Search_Changes_Data(getApplicationContext()).
                                    МетодВычислемБылиИзменениВДанныхВДанныхПоДатам(ТекущаяОбрабоатываеммаяТаблицыДлВнутренгоСинхрониазуииВТАбеле);

                    Log.d(this.getClass().getName(), "ПроверкаБылиИзмененияВБазе "  + ПроверкаБылиИзмененияВБазе[0]);
                    ...
                    if (ПроверкаБылиИзмененияВБазе[0] ==true) {
...
*//*
                            ...
                            try {
                                // TODO: 28.09.2021
                                РезультатЗапускаФоновойСинхронизации = new Class_Engine_SQL(getApplicationContext()).
                                        МетодЗАпускаФоновойСинхронизацииВторойВариантЗапускаемИзТабеля(getApplicationContext(),
                                                "СинхронизацияЛокальная", false,
                                                activity,
                                                null, "ПовторныйЗапускСинхронизации",
                                                "НЕТПоказыватьПослеСинхрониазцииСообщениеВыплывающее");
                                //////
                                // TODO: 22.04.2021  srart JOBschedele
                                Log.d(this.getClass().getName(), " СЛУЖБА  РезультатЗапускаФоновойСинхронизации    "+РезультатЗапускаФоновойСинхронизации);
                    ...
                    try {
                        linkedBlockingQueueДвеТаблитцызапускаПриМинхрониазцииСТАбеля.take();
        ...
        // TODO: 28.09.2021   ПЕРЕХОД НА ДРУГОЕ АКТВИТИ В НЕЗАПИСИМОСТИ БЫЛА СИНХРОНИАЗЦИЯ ИЛИ НЕТ*/

        // TODO: 09.04.2021  конец  запуск синхронизации фоновой по расписанию
        ////
...
/*        // TODO: 09.04.2021 запуск синхронизации фоновой по расписанию
        ...
        boolean       РезультатЕслиСвязьСерверомПередНачаломВизуальнойСинхронизции=false;
            ...
            РезультатЕслиСвязьСерверомПередНачаломВизуальнойСинхронизции=
                    new MODEL_synchronized(getApplicationContext()).УниверсальнайМетодПроверкиПодключениекWIFIДляСлужбыТолькоДляЧата(getApplicationContext());
...
// TODO: 01.07.2021  после локальной обнолвения поробуем вотрунть синхронизацию локальную  в фоне и порстмортрим что будет
            Integer РезультатФоновойСинхронизации =
                    new MODEL_synchronized(getApplicationContext()).МетодЗапускаФоновойСинхронизацииДополнительноНаSingleТабеле(getApplicationContext());
    ...
    // TODO: 01.07.2021 метод дейтрой на СИНГ АКТВИТИ  ДОПОЛНТЕЛЬНЕО ЗАПУСКАЕМ СИНХРОНИАЗУИЮ
...
//TODO #0
            ...
            ///TODO   #8 запускаем метод отработки поднятие клавиатуры
                    ...
                    handler.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (ТекущееПоложенияВТабелеДляСкрола == ОбщееКоличествоСОтрудниковДляСкролаПотабелю) {
                                ...
                                // ТекущееПоложенияВТабелеДляСкрола=  ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.getPosition();
...
/////////////////////////////
    ...
    (SQLiteCursor ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество) {
        ...
        try{
            ...
            observerОдноразоваяВнутриТабеля = new Observer<List<WorkInfo>>() {
                @Override
                public void onChanged(List<WorkInfo> workInfosОдноразовая) {
                        ...
                        workInfosОдноразовая.stream()
                                .filter(ТекущийСтатусОбноразоваяСинхрониазации -> ТекущийСтатусОбноразоваяСинхрониазации.getState().compareTo(WorkInfo.State.SUCCEEDED) == 0
                                        && Objects.nonNull(ТекущийСтатусОбноразоваяСинхрониазации))
                                .flatMap(new Function<WorkInfo, Stream<?>>() {
                                    @Override
                                    public Stream<?> apply(WorkInfo workInfo) {

                                        //
                                        Log.d(this.getClass().getName(), " WorkInfoИнформацияОЗапущенойСлужбеОдноразовая  workInfo " + workInfo.getState().name());


                                        Long   CallBaskОтWorkManagerОдноразового=   workInfo.getOutputData().getLong("ОтветПослеВыполения_MyWork_Async_Синхронизация_Одноразовая",0l);
                                        ...
                                        if (CallBaskОтWorkManagerОдноразового>0) {
                                            ...
                                            ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.requery();
                                            ...
                                            /////
                                            ScrollСамогоТабеля.pageScroll(View.FOCUSABLE_AUTO);
                                            /////
                                        ...
                                        return Stream.empty().sequential();
                                    }
                                })
                                .spliterator().forEachRemaining((ТекущаийСтатусОДНОРАЗОВОЙСИНХ) -> System.out.println(ТекущаийСтатусОДНОРАЗОВОЙСИНХ));
...
// TODO: 29.01.2022
            ...
            WorkManager.getInstance(getApplicationContext()).getWorkInfosByTagLiveData(ИмяСлужбыСинхронизацииОдноразовая).observeForever(observerОдноразоваяВнутриТабеля);
...
// TODO: 29.04.2021 getiD
          ...
          /*  if (ТекущееПоложенияВТабелеДляСкрола==0) {
            ...
            // TODO: 25.09.2021  получем что БЫЛ ЛИ СФАЙ ПО ДАННЫМ

            //////todo дополнтельные данные которые нужно перебросить далеее в другие актвити и ДАЛЕЕ МЕТКА ТАБЕЛЯ
            Intent Интент_ПровеемБылЛиБылСВАПЙП = getIntent();//
            ...
            ПроизошелЛиСфайпПоДаннымСингТабеля = Интент_ПровеемБылЛиБылСВАПЙП.getBooleanExtra("ПроизошелЛиСфайпПоДаннымСингТабеля",false);
            ...
            if (UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников==0l) {
                UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников = ИнтентПришелПослеИзВсехСотрудниовUUID.getLongExtra("РодительскийUUDТаблицыТабель", 0);
            }
...
//поймать ошибку всего классаIOException | MyException e    NumberFormatException
    ...
    private void МетодЗаполненияАлайЛИстаНовымМЕсцевНовогоТабеля(String  МесяцТабеляФинал ,int ЕслиСмещениеВдАнныхДляСкрола) {
        ...
        SQLiteCursor ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество = null;
...
///TODO СЮДА ЗАХОДИМ ПРОСТО БЕЗ СОЗДАНИЕ НОВОГО ЗАГРУЖАЕТ УЖЕ СОЗДАННОЕ ТАБЕЛЯ В БАЗЕ "status_send", "Удаленная"
            ...
            if ( ПроизошелЛиСфайпПоДаннымСингТабеля==false) {
                ...
                ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество=null;

                ////////////////////
                ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество =
                        new MODEL_synchronized(getApplicationContext()).МетодЗагружетУжеготовыеТабеля(КонтекстОдногоСотрудикаВТабеле,
                                UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников,МЕсяцДляКурсораТабелейДЛяПермещения, ГодДляКурсораТабелей);

                ///
                Log.d(this.getClass().getName(), "ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп " + ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getCount());

                ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.moveToFirst();
                ...
                int ИНдексГдеНахолитьсяКолонкаДляФИО=ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getColumnIndex("fio");////name

                Long ПолученыйUUIDФИОСледующийДляПервогоЗапуска=     ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getLong(ИНдексГдеНахолитьсяКолонкаДляФИО);////  String ФИОСледующий

                Log.d(this.getClass().getName(), "ПолученыйUUIDФИОСледующийДляПервогоЗапуска " + ПолученыйUUIDФИОСледующийДляПервогоЗапуска);



                String ФИОСледующий=null;

                ///
                // TODO: 26.08.2021 НОВЫЙ ВЫЗОВ НОВОГО КЛАСС GRUD - ОПЕРАЦИИ
                Class_GRUD_SQL_Operations class_grud_sql_operationsПолучаемНаБазуUUIDфиоПолучаемИзТаблицыФИОИМЯДляSingleтабеля= new Class_GRUD_SQL_Operations(getApplicationContext());
                ///
                class_grud_sql_operationsПолучаемНаБазуUUIDфиоПолучаемИзТаблицыФИОИМЯДляSingleтабеля. concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций.put("СамFreeSQLКОд",
                        " SELECT name  FROM fio  WHERE  uuid = '" + ПолученыйUUIDФИОСледующийДляПервогоЗапуска + "' ;");
                ///////


                // TODO: 12.10.2021
                SQLiteCursor            Курсор_ПолучаемИмяСотрудникаИзТаблицыФИО= (SQLiteCursor) class_grud_sql_operationsПолучаемНаБазуUUIDфиоПолучаемИзТаблицыФИОИМЯДляSingleтабеля.
                        new GetаFreeData(getApplicationContext()).getfreedata(class_grud_sql_operationsПолучаемНаБазуUUIDфиоПолучаемИзТаблицыФИОИМЯДляSingleтабеля. concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций,
                ...
                if(Курсор_ПолучаемИмяСотрудникаИзТаблицыФИО.getCount()>0){
                    ...
                    ФИОСледующий=Курсор_ПолучаемИмяСотрудникаИзТаблицыФИО.getString(0);
                ...
                // TODO 24.09.2021  результат оплученный ФИО
...
// TODO: 28.04.2021 вперед
                ...
                int ИНдексГдеНаходитьсяUUIDПервыйЗапуск=         ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getColumnIndex("uuid");


                UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников= ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getLong(ИНдексГдеНаходитьсяUUIDПервыйЗапуск)    ;
                ...
                // TODO: 24.09.2021  САМО ЗАПОЛЕНЕНИЕ  после получение данных и дем  уже визуально заполнять ТАБЕЛЬ ДНИ Д1,Д2,Д3,Д4,Д5,Д6


                МетодЗаполенияТабелемСотрудникаДнемиИДаннымиКним(ЕслиСмещениеВдАнныхДляСкрола,
                        ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество,
                ...
                // TODO: 25.09.2021  КОД КОГДА ПРОШЕЛ СФАЙП




/////////////////////////
            } else  if  (  ПроизошелЛиСфайпПоДаннымСингТабеля==true ){
                ...
                if (UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников==0) {
                    ...
                    ////////////
                    ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество =
                            new MODEL_synchronized(getApplicationContext()).МетодЗагружетУжеготовыеТабеляПриСмещенииДанныхСкроллПоДАнным(КонтекстОдногоСотрудикаВТабеле,
                                    ЦифровоеИмяНовгоТабеля,МЕсяцДляКурсораТабелейДЛяПермещения, ГодДляКурсораТабелей);
                    ...
                    ////////////////////
                    ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество = new MODEL_synchronized(getApplicationContext()).МетодЗагружетУжеготовыеТабеля(КонтекстОдногоСотрудикаВТабеле,
                            UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников,МЕсяцДляКурсораТабелейДЛяПермещения, ГодДляКурсораТабелей);
                ...
                ///
                Log.d(this.getClass().getName(), "ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество " + ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество
                        + " ТекущееПоложенияВТабелеДляСкрола " +ТекущееПоложенияВТабелеДляСкрола);


                // TODO: 12.05.2021  перемещение по данным вперед

                /// ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.moveToFirst();

                Integer ИндексКоличествоЗначениейВКурсоре=ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getCount();

                if (ЕслиСмещениеВдАнныхДляСкрола <= ИндексКоличествоЗначениейВКурсоре) {

                    // TODO: 25.09.2021 САМО СМЕЩЕНИЕ ДАННЫХ

                    ////
                    ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.move(ТекущееПоложенияВТабелеДляСкрола);  //ТекущееПоложенияВТабелеДляСкрола


                    ////
                    //     ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.moveToPosition(ТекущееПоложенияВТабелеДляСкрола-1);///          ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.move(ТекущееПоложенияВТабелеДляСкрола);

                    //  ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.moveToNext();



                    int ИНдексГдеНаходитсьяКолонкаUUIDТабеляПриСфайпе=ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getColumnIndex("uuid");


                    UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников=     ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getLong(ИНдексГдеНаходитсьяКолонкаUUIDТабеляПриСфайпе);
                    ...
                    int ИНдексГдеНаходитсьяКолонкаНаходимФИОПриСфайпе=ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getColumnIndex("fio");


                    Long ПолученоеЗначениеФИОПриСфайпе=     ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.getLong(ИНдексГдеНаходитсьяКолонкаНаходимФИОПриСфайпе);


                    ///
                    Log.d(this.getClass().getName(), "ПолученоеЗначениеФИОПриСфайпе  " +ПолученоеЗначениеФИОПриСфайпе);


                    String ФИОСледующий=null;


                    // TODO: 24.09.2021  ПОЛУЧСЕАМ ФИО
                    ///
                    ...
                    ///
                    class_grud_sql_operationsПолучаемНаБазуUUIDфиоПолучаемИзТаблицыФИОИМЯДляSingleтабеля. concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций.put("СамFreeSQLКОд",
                            " SELECT name  FROM fio  WHERE  uuid = '" + ПолученоеЗначениеФИОПриСфайпе + "' ;");
                    ...
                    SQLiteCursor            Курсор_ПолучаемИмяСотрудникаИзТаблицыФИО=
                            (SQLiteCursor) class_grud_sql_operationsПолучаемНаБазуUUIDфиоПолучаемИзТаблицыФИОИМЯДляSingleтабеля.
                                    new GetаFreeData(getApplicationContext()).getfreedata(class_grud_sql_operationsПолучаемНаБазуUUIDфиоПолучаемИзТаблицыФИОИМЯДляSingleтабеля.
                    ...
                    //////
                    Log.d(this.getClass().getName()," Данные в курсор не загруженны ( отсутвуют:) -- МЕсяцДляКурсораТабелей "+МЕсяцДляКурсораТабелей+" ГодДляКурсораТабелей " +ГодДляКурсораТабелей +
                            " UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников " +UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников);
...
// TODO: 29.01.2022 ПРИ ИЗМЕНЕНИ МЕНЯЕМ ДАННЫЕ В БАЗЕ В ТАБЕЛЕ
            ...
            МетодПриИзмениеДанныхВБазеМенемВнешнийВидТабеляObserver(ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество);


            Log.d(this.getClass().getName(), "ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество " +
                    ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество);
            ...
            ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайпТОлькоОбшееКоличество.close();
    ...
    private void МетодЗаполенияТабелемСотрудникаДнемиИДаннымиКним(int еслиСмещениеВдАнныхДляСкрола,
        ...
        try{
            ...
            if (ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.getCount()>0) {
                ...
                // НазваниеЗагруженногТАбеля = ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.getString(РасположениеИмениТабеля); ///строго имя
                ...
                СтаттусТабеля = ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.getInt(ИндексГдеНаходитьсяСтутсПроведенных); ///строго имя

                ///todo
                if (СтаттусТабеля==null) {
                    //
                    СтаттусТабеля=0;
                }
                ...
                //   int НазваниеСтобика= ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.getColumnIndex("sumofhours");

                //  ОбщееКоличествоЛюдейВТабелеТекущем =  ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.getString(НазваниеСтобика);
                /////////
       ...
       НазваниеДанныхВТабелеСНИЛС=new TextView(this);
            ОбщееКоличествоЧасовСотрудникаВТабелеСотудников=new TextView(this);*/
                ///TODO future
  ...
  /*          if (!Build.BRAND.toString().contains("Redmi") ) {
            ...
            КонтентТабеляКоторыйМыИБудемЗаполнятьВнутриЦикла.setForegroundGravity(Gravity.CENTER_HORIZONTAL);*/
                ...
                SQLiteCursor            Курсор_ПолучаемИмяСотрудникаИзТаблицыФИО= (SQLiteCursor) class_grud_sql_operationsПолучаемНаБазуUUIDфиоПолучаемИзТаблицыФИОИМЯДляSingleтабеляДва.
                        new GetаFreeData(getApplicationContext()).getfreedata(class_grud_sql_operationsПолучаемНаБазуUUIDфиоПолучаемИзТаблицыФИОИМЯДляSingleтабеляДва. concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций,
                        Class_Engine_SQLГдеНаходитьсяМенеджерПотоков.МенеджерПотоков
                        ,Create_Database_СсылкаНАБазовыйКласс.getССылкаНаСозданнуюБазу());
                ...
                ////ФИОДляТабеляНаАктивти = ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.getString(РасположениеИмениСотркдника);
/*

            int РасположениеСНИЛС = ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.getColumnIndex("snils");

            String СНИЛСДляТабеляНаАктивти = ГлавныйКурсорКогдаМыКликаемПоСпискуТабелейИлиСфайп.getString(РасположениеСНИЛС);
...
///
                ...
                ОбщееКоличествоЛюдейВТабелеТекущем = new Class_Engine_SQL(getApplicationContext()).
                ...
                // TODO: 25.09.2021
                Log.d(MODEL_synchronized.class.getName()," ОбщееКоличествоЛюдейВТабелеТекущем  "+ОбщееКоличествоЛюдейВТабелеТекущем  +
                        "  ТекущееПоложенияВТабелеДляСкрола "  +ТекущееПоложенияВТабелеДляСкрола);
                ///TODO запускаем метод ПОСЛЕ УСПЕШНОЙ ГЕНЕРАЦИИ ТАБЕЛЯ
...
// TODO: 08.10.2021 повышаем версию
                             ...
                             /*       Class_GRUD_SQL_Operations        class_grud_sql_operationsПовышаемВерсиюДанныхПриСозданеииИзШаблонаСотрудника=new Class_GRUD_SQL_Operations(getApplicationContext());
                                    ...
                                    Long РезультатУвеличинаяВерсияДАныхВнутмиТАбеля=0L;
                                    ...
                                    РезультатУвеличинаяВерсияДАныхЧата=         class_grud_sql_operationsПовышаемВерсиюДанныхПриСозданеииИзШаблонаСотрудника. new ChangesVesionData(getApplicationContext()).
                                            МетодПолученияУвеличинойВесрииДанныхДляТекущейВнутренейтаблицы_ПоПолю_current_table("tabel","localversionandroid_version",getApplicationContext());
*/
...
/*
                ...
                // TODO: 07.09.2021    __old
                                ...
                                Курсор_ЗагружаемИнформациюПоФИО = new Class_Engine_SQL(getApplication()).КурсорУниверсальныйДляБазыДанных("viewtabel",
...
////TODO сам метод запуска обновления
            if (КонтейнерЗаполненияДаннымиПриЛокальномОбновлении.size() > 0) {///если в контенер заполнилься то начинаем обновление
                ...
                if (ПолучениеЗначениеСтолбикUUID != null) {
                    ...
                    try {
                        ...
                        if (РузультатМетодОпределяемуЗаписиКакойСтатусУдаленныйИлиНет == false) {
                            ...
                            РезультатЛокальногоОбновления = new Class_Engine_SQL(getApplicationContext()).
                ...
                if (РезультатЛокальногоОбновления > 0 ) {
                    ...
                    ОбщееКоличествоЛюдейВТабелеТекущем = new Class_Engine_SQL(getApplicationContext()).
                            МетодПосчётаЧасовСотрудниковВТабеле(КонтекстОдногоСотрудикаВТабеле,
                                    UUIDТабеляПослеУспешногоСозданиеСотрудникаВсехСотридников, МЕсяцДляКурсораТабелейДЛяПермещения, ГодДляКурсораТабелей);
...
//////TODO ГЛАВНЫЙ КУРСОР ДЛЯ НЕПОСРЕДТСВЕНОГО ЗАГРУЗКИ СОТРУДНИКА
            // TODO: 08.09.2021 ____ old
                        Курсор_УзнаемСтатусЗаписиДляЛокальногоОбновленияУдаленноеИлиНет = new Class_Engine_SQL(getApplicationContext()).КурсорУниверсальныйДляБазыДанных("tabels",
...
/////////кнопка
    ...
    private void МетодПереходаНаМеткиТабеля(HashMap<String,Long> ХэшЛовимUUIDIDНазваниеСтолбикаЛокальныйСловоВставитьТабель) {
        ...
        try{
            ...
            IntentЗапускаемСправочникДЛяСловТабеля.setClass(КонтекстОдногоСотрудикаВТабеле, MainActivity_Metki_Tabels.class); // Т
...
////todo конец фильаного сообщения о удалени самого табеля
    ...
    void МетодЗапускаетСотрудниковПослеУспешногоУдалениеСотрудника() {
        ...
        try{
            ...
            Интент_ЗапускСозданиеНовогоСотрудника.setClass(КонтекстОдногоСотрудикаВТабеле, MainActivity_List_Employees_Current_Tabel.class); //  ТЕСТ КОД КОТОРЫЙ ЗАПУСКАЕТ ACTIVITY VIEWDATA  ПРОВЕРИТЬ ОБМЕН
            // передача объекта с ключом "hello" и значением "Hello World"
