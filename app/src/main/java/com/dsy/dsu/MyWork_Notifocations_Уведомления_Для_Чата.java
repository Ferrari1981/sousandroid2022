...
public class MyWork_Notifocations_Уведомления_Для_Чата extends Worker {
    ...
    String ИмяСлужбыУведомленияДляЧата = "WorkManager NOtofocationForChats";
   ...
   /*     messagingStyleДляОдноразовыеУведомления=   new NotificationCompat.MessagingStyle(getApplicationContext().getResources().getString(R.string.action_settings))
    ...
    public Result doWork() {
        ...
        try {
            ...
            ИнформацияОЗапущенойСлужбе_Уведомления_ДляЧатаОдноразовое = WorkManager.getInstance(Контекст.getApplicationContext()).getWorkInfosByTag(ИмяСлужбыУведомленияДляЧата).get().get(0);
            ...
            if (ИнформацияОЗапущенойСлужбе_Уведомления_ДляЧатаОдноразовое!=null ) {
                ...
                Log.w(Контекст.getClass().getName(), " Внутри метода public Result doWork()   MyWork_Notifocations_Уведомления_Для_Задачи оДНОРАЗОАПЯ  ИмяСлужбыУведомленияДляЧата" +
...
// TODO: 17.11.2021 БЛОК КОДА РЕАЛИЗАЦИЯ БУДУШЕГО ЗАПУСКА ПРИ НАЖАТИИ НА УВЕДОСЛЕНИЕ ИЛИ НА КНОПКИ ЗАПУСКАЕТ С УВЕДОМЛЕНИЯ РАЗЛИЧНЫЕ ДЕЙСТВИЯ
            ...
            PendingIntent ЗапускКОдаЧтоПОльзовательЗАкрытиеЧата = new SubClass_Starting_chahge_status_public_Chat_Класс_ДляЧата(getApplicationContext()).
                    МетодЗапускаЧатаЗакваваемВнутриУведомленияPendingIntent(PROCESS_ID_УведомленияОдноразовые, ИмяСлужбыУведомленияДляЧата,
                            0,
                            0, "");
            ...
            PendingIntent ЗапускПриКликеКодаИзНадоперейтиСУведомленияВСамЧат = new SubClass_Starting_chahge_status_public_Chat_Класс_ДляЧата(getApplicationContext()).
                    МетодПриКликеЗапускаЗаданияИзСамогоУведомленияЧатаПереход(PROCESS_ID_УведомленияОдноразовые, ИмяСлужбыУведомленияДляЧата,
                            0,
                            0, "");
...
/*
            ...
            Log.w(Контекст.getClass().getName(), " ПОСЛЕ ОТРАБОТКИ МЕТОДА ....Внутри метода public Result doWork() BroadcastReceiver_Sous_Notificatioons_For_Tasks  ОДНОРАЗОВАЯ " + ИмяСлужбыУведомленияДляЧата + "\n"
                    + " getState  " +
